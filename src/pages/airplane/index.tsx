...
const AirplaneGeneralGame = () => {
  ...
  // --- LOGIC EXIT (POST PLAY COUNT) ---
  const handleExit = async () => {
    try {
        if (id) {
            // Req ke backend khusus airplane untuk nambah play count
            await api.post(`/api/game/game-type/airplane/${id}/play`);
        }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-800 flex items-center justify-center p-4 select-none">
      <div className="relative rounded-xl shadow-2xl overflow-hidden ring-8 ring-slate-700">
        ...
        {/* Layer 2: UI Overlay / HUD (z-50) */}
        {/* pointer-events-none di container agar klik tembus ke canvas */}
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-50">
            <div className="flex justify-between items-start p-6">
                
                {/* Kiri: Stats & Pause Button */}
                <div className="flex flex-col gap-3 pointer-events-auto">
                    ...
                    {gameState === 'playing' && (
                        <Button 
                            size="icon" 
                            className="rounded-xl h-12 w-12 bg-amber-400 hover:bg-amber-500 text-slate-900 border-b-4 border-amber-700 active:border-b-0 active:translate-y-1 transition-all shadow-lg cursor-pointer pointer-events-auto"
                            onClick={pauseGame}
                        >
            ...
            {/* Hint Controls (Pojok Kanan Bawah) */}
            <div className="absolute bottom-4 right-4 bg-black/40 text-white/80 px-3 py-2 rounded-lg text-sm font-mono backdrop-blur-sm border border-white/10 text-right pointer-events-none">
                 CONTROLS<br/>
                 <span className="text-yellow-300 font-bold">↑ UP</span><br/>
                 <span className="text-yellow-300 font-bold">↓ DOWN</span>
            </div>
        </div>
